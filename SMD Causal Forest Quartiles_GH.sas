...
*import data set with causal forest results from R;
proc import datafile='filepath'
out=ntiles
dbms=csv replace;
guessingrows=max;
run;
/*NOTE: The data set WORK.NTILES has 218604 observations and 152 variables.*/

*confirm no ntile missing;
proc sql;
select patientid 
from ntiles
where ntile = "";
...
*create quartile grouping of quartile 4 vs all others;
*create numeric since &groupvar should be num;
data ntiles;
set ntiles;
if ntile = '4' then ntile_4th = 1;
	else ntile_4th = 0;
	run;
...
*create age_over75 var and mutually exclusive cancer vars;
data ntiles2;
set ntiles;
if age = . then age_over75 = .;
	else if age > 75 then age_over75 = 1;
		else age_over75 = 0;

if cancer_met = 1 and cancer_nonmet = 1 then cancer_nonmet = 0;

run;

proc freq data=ntiles2;
tables cancer_met*cancer_nonmet;
run;
proc means data=ntiles2 min max;
class age_over75;
var age;
run;
...
%stddiff( inds = ntiles2, 
                      groupvar = ntile_4th, 
                      numvars = age, 
                      charvars = SIRS_TEMP SIRS_PULSE SIRS_RR SIRS_WBC
AOD_LUNG AOD_KIDNEY AOD_LIVER AOD_HEME AOD_LACTATE pressor_in_72hr HTN
MALE AGE_over75 CHF CARDIC_ARRHYM VALVULAR_D2 PULM_CIRC PVD PARALYSIS
...
*Hospitalizations;
proc sql;
select count(distinct uniqid), ntile_4th
from ntiles2
group by ntile_4th;
quit;

*Age Cont;
proc means data=ntiles2 median p25 p75 maxdec=2;
var age;
class ntile_4th;
run;
...
*Cat Vars;
proc freq data=ntiles2;
tables (age_over75 male sirs_pulse sirs_wbc sirs_rr sirs_temp
	aod_lactate aod_kidney aod_heme pressor_in_72hr aod_liver aod_lung
	cancer_met liver lymphoma coag wtloss anemia_new neuro pud
	valvular_d2 obesity fen pulm_circ cardic_arrhym pvd chf dm_uncomp
	cancer_nonmet dm_comp renal paralysis pulm ra hypothyroid htn)*ntile_4th / nopercent norow nocum;
run;

proc tabulate data=ntiles2 ;
class ntile_4th age_over75 male sirs_pulse sirs_wbc sirs_rr sirs_temp
	aod_lactate aod_kidney aod_heme pressor_in_72hr aod_liver aod_lung
	cancer_met liver lymphoma coag wtloss anemia_new neuro pud
	valvular_d2 obesity fen pulm_circ cardic_arrhym pvd chf dm_uncomp
	cancer_nonmet dm_comp renal paralysis pulm ra hypothyroid htn ;

table age_over75 male sirs_pulse sirs_wbc sirs_rr sirs_temp
	aod_lactate aod_kidney aod_heme pressor_in_72hr aod_liver aod_lung
	cancer_met liver lymphoma coag wtloss anemia_new neuro pud
	valvular_d2 obesity fen pulm_circ cardic_arrhym pvd chf dm_uncomp
	cancer_nonmet dm_comp renal paralysis pulm ra hypothyroid htn, all(colpctn) ntile_4th*(n colpctn) ;
run;
